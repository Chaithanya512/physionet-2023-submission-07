...
def train_model(features,outcomes,cpcs):
    ...
    grid_search_clf= GridSearchCV(xgb_classifier, param_grid_xgb, cv=3,verbose = 5)
...
def get_features(data_folder, patient_id):
    ...
    eeg_data , sampling_frequency1 =   get_eeg_data(data_folder, patient_id)
    eeg_features = get_eeg_features2(eeg_data,sampling_frequency1)
    ts_features = get_eeg_features(eeg_data)
    # Extract features.
    return np.hstack((patient_features, eeg_features.flatten() ,ts_features.flatten() ))
...
def get_eeg_features2(data, sampling_frequency):
    if data is None:
        return float("nan")*np.ones(72)
...
def get_eeg_features(eeg_data):
    ...
    features = get_tsfresh_features(eeg_data)
